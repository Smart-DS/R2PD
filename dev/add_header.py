...
LICENSE_FILE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
ARGS['filename'] = ([], {'type': str,
                         'help': """the python filename (or directory with
 [-d] or [--folder]) to add the license file to"""})

ARGS['-d'] = (['--folder'], {'action': 'store_true',
                             'default': False,
                             'help': """(optional) The provided first parameter
 is a directory. All .py files in the directory will add the license.
 Default: False"""})

ARGS['-l'] = (['--license'], {'default': LICENSE_FILE,
                              'help': """(optional) The filename of the
license to add to the Python files"""})

ARGS['-r'] = (['--recursive'], {'action': 'store_true',
                                          'default': False,
                                          'help': """(optional) Recursively add
 license to files. Only works if [-d] or [--folder] is specified."""})

ARGS['-x'] = (['--remove'], {'action': 'store_true',
                                       'default': False,
                                       'help': """(optional) Remove the license
 from the given file only, do not add the provided license."""})


def get_header(s):
    '''
    Splits .py file into its header and body. If no header is found on line 0,
    one is created.
    '''
    header = ''
    body = ''
...
def add_license(header, lic):
    if has_license(header):
        msg = ('There is already a license in the provided header.',
               'Please rem_license prior to adding a new one.')
